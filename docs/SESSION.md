...
- **Tag system (ADR-104) — implemented in an isolated worktree, Part B of a
  ...
  - **Could not verify**: `tsc --noEmit` / `npm test` / `npm run dev` all
    blocked by this machine's AppLocker policy (documented CLAUDE.md
    constraint) — reviewed every changed file by hand instead. No
    Playwright/TEST-household pass attempted either (out of budget for
    this pass) — flagging honestly rather than claiming it. Recommend a
    `tsc`/`vitest` pass plus a Playwright smoke test against the TEST
    household (tag a plain transaction; tag one line of a split; edit that
    split's amount and confirm the tag survives; delete a tag and confirm
    `transaction_tags` cleans up; drill into Transactions from the Tags
    screen) before trusting this in production.
  - Not committed — left for the coordinating session/user to review and
    commit, per this repo's "never commit unless asked" convention.
