...
- **Tag system (ADR-104) — implemented in an isolated worktree, Part B of a
  3-part plan (income-received QOL default / tags / Year in Review).**
  Free-form labels on any transaction, or one specific split line, with
  multiple tags per line and a new totals+drill-down screen.
  - Schema: `scripts/migrations/2026-09-15-add-tags.sql` (+ `.verify.sql`) —
    `tags` (household_id, name, icon, color) + `transaction_tags`
    (composite PK, both FKs `on delete cascade`), same shape as ADR-005's
    `institution_categories`, RLS scoped through `tags.household_id`. Not
    yet applied — user runs it manually in the Supabase SQL Editor.
  - `src/lib/tags.ts` (new): `useTags`/`useCreateTag`/`useUpdateTag`/
    `useDeleteTag`/`useTransactionTags`/`useSetTransactionTags`, mirroring
    `useInstitutionCategories`'s shape.
  - `src/components/TagPicker.tsx` (new, chip multi-select, modeled on
    `InstitutionDialog.tsx`'s category picker) and `TagDialog.tsx` (new,
    reuses `IconPicker`/`ColorPicker` already exported from
    `app.categories.tsx`).
  - `src/components/SplitLinesEditor.tsx`: `SplitRow` gains `tagIds`, a
    compact `TagPicker` per line.
  - `src/lib/data-hooks.ts`: `useSaveSplitTransaction` now `.select("id")`s
    its insert and writes `transaction_tags` per line, index-correlated;
    `useUpsertTransaction` now actually returns the saved row (previously
    discarded) so a plain create can tag its own new id.
  - `src/components/AddTransactionFab.tsx` and `src/routes/app.transactions.tsx`
    (`TransactionDetail`, `SplitTransactionDetail`) wired for both directions
    — tag at creation and re-tag on edit. `SplitTransactionDetail` seeds each
    line's tags from its current transaction id before every save, since
    `useSaveSplitTransaction` deletes+reinserts the whole group on edit —
    the exact mechanism that silently dropped `institution_id` on split
    edits before the 2026-09-13 fix; same fix shape applied here so tags
    survive a split edit.
  - New `src/routes/app.tags.tsx` (totals + drill-down via
    `tx-filter-store.ts`'s `tagId` field, reusing `app.spending.tsx`'s
    pattern), linked from `app.more.tsx`.
  - Added a Tags filter to the Transactions filter panel and tag chips on
    list rows/detail.
  - **`src/routeTree.gen.ts` hand-edited** to register `/app/tags` — this
    file is TanStack Router's auto-generated output and is normally never
    hand-edited; done only because the codegen tool can't run here
    (AppLocker blocks `node_modules\.bin\*`). Mirrored the exact pattern of
    every other route across all 8 touch points in the file and verified
    by grep that the count matches an existing route's. **This should be
    regenerated by the real tool** (`npm run dev` or the TanStack Router
    CLI) in a Codespace at the first opportunity rather than trusted long-term.
  - **Verification update (coordinating session, post-merge)**: discovered
    `node node_modules/typescript/bin/tsc` / `node node_modules/vitest/vitest.mjs`
    bypass the AppLocker `.bin` shim block (only the shims themselves are
    blocked, not the underlying JS entry points) — this machine's
    "cannot verify builds locally" constraint is narrower than documented.
    Ran both against this branch: **`tsc --noEmit` clean** (one pre-existing,
    unrelated `@capacitor/browser` error also present on `main`) and
    **199/199 tests pass**. Manually reviewed every changed file by hand as
    well (routeTree.gen.ts hand-edit checked line-by-line against every
    other route's pattern; RLS policy checked live against
    `institution_categories`' actual policy via the MCP). Still no
    Playwright/TEST-household pass — recommend one (tag a plain
    transaction; tag one line of a split; edit that split's amount and
    confirm the tag survives; delete a tag and confirm `transaction_tags`
    cleans up; drill into Transactions from the Tags screen) before
    trusting this in production. Committed to `main` locally; **not
    pushed** — the migration must be run first (the code now references
    `tags`/`transaction_tags`, which don't exist until it is).
...
- **Parts A/B/C merged into `main` locally (coordinating session).** Both
  worktree branches independently touched `routeTree.gen.ts`/`app.more.tsx`
  (auto-merged cleanly, non-overlapping route entries) and `SESSION.md`
  (resolved by hand, concatenating both notes). Ran `tsc --noEmit` and
  `vitest run` again against the **combined** tree post-merge: clean (same
  one pre-existing `@capacitor/browser` error), all tests pass. **Not
  pushed** — the ADR-104 migration (`scripts/migrations/2026-09-15-add-tags.sql`)
  must be run by the user first, since the merged code already references
  `tags`/`transaction_tags`. Open items carried forward: the `routeTree.gen.ts`
  hand-edits should be replaced by a real codegen run once available; a
  Playwright/TEST-household pass for the tag system hasn't been done; the
