...
- Added `foreignObjectRendering: true` to the html2canvas-pro options in src/lib/snapshot.ts. This delegates rendering to the browser's native SVG foreignObject painter, fixing the remaining styling gaps (card backgrounds/shadows, app font, colored progress ring arc) that html2canvas-pro's manual CSS reimplementation missed. Re-verified with a headless capture of the same snapshot primitives: exported PNG now shows the green gradient hero card, destructive-tint overdue card, white upcoming cards with shadows, correct sans-serif typography, and a filled red 68% ProgressRing arc. Temporary test route removed after verification.
