...
- 2026-08-26 — Regenerated `src/routeTree.gen.ts` with the Codespace's
  `@tanstack/router-plugin` and committed it (own commit). The tracked copy was
  generated by an older plugin (the Windows AppLocker env couldn't run vite),
  so every build/test re-sorted the route declarations and showed the file
  dirty. Pure reordering — 541 lines, same routes. Working tree now stays clean
  across build/test runs.
